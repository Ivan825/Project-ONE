...
registers strong apparent steering toward false and replayed self-models,
and we show it is fully accounted for by intrinsic dynamics measured on
untreated twin trajectories, a direct caution for empirical tests of
performative prediction; (3) evidence for what broadcasts \emph{do} change:
...
population mean declines most strongly under unreliable, high-impact
self-models. Figure~\ref{fig:loop} summarizes the design.
...
prediction \cite{perdomo2020performative}, including networked multi-agent
settings \cite{wang2023network} and populations that decide whether to trust
predictions of collective outcomes \cite{gois2024performative};
...
behavior \cite{park2023generative}; shared dashboards describing such
populations' collective state are a natural coordination design there, and
the present rule-based setting provides causally clean baselines for it.
...
highest-degree neighbor (ties by id) with probability $\gamma_i$ --- note
$g$ scales action propensities only, not this target-selection probability
--- else a uniform neighbor; and new links attach to a neighbor-of-a-neighbor with
...
(configuration-model) null most of the clustering excess is accounted for by
the degree sequence (ratios $1.0$--$1.6$), so we make no small-world claim:
feedback acts on a small, dense, degree-heterogeneous clustered graph rather
than a homogeneous mixing population.

\subsection{Reproducibility}
The simulation is deterministic given (configuration, seed): one PRNG drives
all behavioral choices, a separate dedicated stream generates broadcast
signals (so conditions differ only in signal \emph{content}, never in RNG
consumption), agents act in sorted order, and a state hash certifies replay;
...
against a matched reference of the same type, defining the causal steering
effect $\Delta P = P_{\mathrm{actual}} - P_{\mathrm{passive}}$
...
The passive counterfactual dissolves this picture. For each treated run we
score the paired-seed \emph{untreated} trajectory (A, or the identical B)
against a matched reference of the same type, post hoc: the inverted mirror
of its own state, the same replayed source trajectory, or the same noise
...
R vs N: $p=0.95$). Raw $P$ is therefore dominated by intrinsic
dynamics --- post-shock relaxation and drift toward typical states --- that
align best with realistic references (the replayed trajectory shares the
ecology's attractor and the common shock time; excluding the shock
transition changes medians by $<2\%$) and worst with unstructured noise.
...
the largest raw pull --- it was never response-driven. We conclude there is \emph{no evidence of content-specific attraction
toward broadcast content}; the small negative $\Delta P$, similar across
signal types, indicates broadcasts mildly damp the intrinsic drift.
This is the paper's methodological result: a statistic of the kind natural
in empirical performativity studies registers large, internally consistent,
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\textwidth]{network_pair.png}
...
property of the lie--response pair, not of the lie alone.
...
(neutral drift), truth $-0.10$, noise $-0.34$ (paired $r=-0.94$,
$p\!\approx\!2\times10^{-7}$), false
$-0.42$ (paired $\Delta$median $-0.336$ $[-0.456,-0.281]$, $r=-1.00$,
$p\!\approx\!1.9\times10^{-9}$). The sweep reveals a
dose--response: the decline under the false broadcast deepens from $-0.12$
...
gradient (within-run $\rho\approx+0.05$ between $\gamma_i$ and offspring
count under F) and not shock-mediated ($-0.40$ without the shock vs $-0.39$
with it; shock victims' mean $\gamma$ only $+0.03$ above the mean). The
...
\begin{figure}[t]
\centering
\includegraphics[width=0.5\textwidth]{fig8_trait_evolution.png}
...
median $\approx 40$ steps throughout). Within the regimes tested,
resilience appears demographic --- fast regrowth --- rather than
informational; we report this pre-specified null as observed.
...
$[0.119, 0.247]$, $r=1.00$, $p=0.008$), and the distrust ordering is intact
(F vs.\ A: $\Delta$median $-0.299$ $[-0.388, -0.212]$, $r=-1.00$, $p=0.008$;
N vs.\ A: $-0.219$, $r=-0.89$, $p=0.023$). The principal phenomena therefore
persist under a $2\times$ increase in system scale.
...
equivalent to no telemetry} while raising no alarms. Documented episodes of
measurement reshaping the measured system, such as ranking reactivity in
institutions \cite{espeland2007rankings}, are natural empirical targets ---
with the passive-counterfactual check as the entry requirement. For
multi-agent AI, where populations of language-model agents are increasingly
studied and deployed \cite{park2023generative} and shared dashboards of
collective state are a natural design, the evolution-of-distrust result
suggests that persistent misreporting does not merely misdirect a
...
collective self-model declines across generations, more strongly with
increasing self-model unreliability and impact, making a population's trust in its
own self-model an evolving quantity that unreliable observers spend down.
Future work: learned response policies and language-model agents.

\smallskip
\noindent\textbf{Reproducibility.} Code, campaign configurations, all
analysis scripts, pinned dependency versions, and deterministic
regeneration of every run are public at
\texttt{github.com/Ivan825/Project-ONE}, tag \texttt{cn2026-submission};
stored run records and replay-source trajectories ship as release assets on
that tag.
...
mechanism design. arXiv:2408.05146 (2024)
